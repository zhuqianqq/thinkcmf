...
class AdminArticleController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            //增加json内容的代码片段  start
            $data['post']['json_content'] = '';
            $_data = [];
            if(count($data['json_pics']) != 0){

                for ($i=0; $i < count($data['json_pics']); $i++) { 

                    $_data[$i]['img'] = $data['json_pics'][$i];
                    $_data[$i]['json_title'] = $data['json_title'][$i] ?? '';
                    $_data[$i]['json_subtitle1'] = $data['json_subtitle1'][$i] ?? '';
                    $_data[$i]['json_subtitle2'] = $data['json_subtitle2'][$i] ?? '';
                }
                

                $data['post']['json_content'] = json_encode($_data);
            }
            //增加json内容的代码片段 end
